...
public class ServiceTransactionDAO {
    ...
    public static ObservableList<ServiceTransaction> searchTransaction(String transactionName) throws SQLException {
        ...
        String selectStmt = "SELECT p.id AS 'id', p.name AS 'name', p.dateBirth AS 'dateBirth', " +
                ...
                "l.id = p.deletedBy " +
                "WHERE p.name LIKE '%" + transactionName + "%' AND (p.deletedAt IS NULL AND cs.deletedAt IS NULL AND ps.deletedAt IS NULL);";
        ...
        try {
            ...
            ObservableList<ServiceTransaction> pet = FXCollections.observableArrayList();
            try {
            ...
            System.out.println("While searching an pet with Id : " + transactionName + ", an error occurred: " + ex);
            //Return Exception
            throw ex;
        }
    }
    ...
    //SELECT Pets
    public static ObservableList<ServiceTransaction> searchPets() throws SQLException, ClassNotFoundException {
    ...
    public static ObservableList<ServiceTransaction> getPetList(ResultSet rs) throws SQLException, ClassNotFoundException {

        //Declare a observable List which comprises of Pet Objects
        ObservableList<ServiceTransaction> petList = FXCollections.observableArrayList();

        while (rs.next()) {

            ServiceTransaction pet = new ServiceTransaction();
//            pet = new Pet();
//            pet.setId(rs.getInt("id"));
//            pet.setName(rs.getString("name"));
//            pet.setDateBirth(rs.getDate("dateBirth"));
